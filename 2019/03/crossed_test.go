...
func TestMove(t *testing.T) {
	...
	for _, c := range tc {
		...
		if reflect.DeepEqual(g, c.result) == false {
			t.Errorf("%v != %v", g, c.result)
		}
	}
}
...
func TestExamples(t *testing.T) {
	tc := []struct {
		...
		{
			move:    "R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83",
			dist:    159,
			latency: 610,
		},
		{
			move:    "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51\nU98,R91,D20,R16,D67,R40,U7,R15,U6,R7",
			dist:    135,
			latency: 410,
		},
	}
	...
	for _, c := range tc {
		paths := strings.Split(c.move, "\n")
		dist, latency, err := Check(paths)
		if err != nil {
			t.Error(err)
		}
		if dist != c.dist {
			t.Errorf("%d != %d", dist, c.dist)
		}
		if latency != c.latency {
			t.Errorf("%d != %d", latency, c.latency)
		}
	}
}
